...
// representations as "Field Name Sources". In the above examples, the User struct's EmailAddress has a "struct" field name source ("EmailAddress"),
// and a "json" field name source ("email_address"). Thus, if you want to patch a struct from a map built from JSON bytes, you need to add json tags
// AND configure your Patcher to use the struct's "json" field name sources. In the second example above, you can see the Patcher has been configured
// with the "json" field name source and thuse can use a JSON-derived map to patch.
//
// Patch Results
//
